...
const inserMessage = `-- name: InserMessage :one
...
type InserMessageParams struct {
	RoomID  uuid.UUID
	Message string
}

func (q *Queries) InserMessage(ctx context.Context, arg InserMessageParams) (uuid.UUID, error) {
	row := q.db.QueryRow(ctx, inserMessage, arg.RoomID, arg.Message)
	var id uuid.UUID
	err := row.Scan(&id)
	return id, err
}
